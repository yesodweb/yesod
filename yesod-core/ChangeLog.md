...
* Extend `ResourceParent` to include route attributes. Replace `frParentPieces` with `frParentDetails` to gather a parent's pieces and attributes. [#1911](https://github.com/yesodweb/yesod/pull/1911) These attributes are inherited by the parent's child leaves at parse time (as before); a parent's *own* attribute set is not surfaced as a separate `routeAttrs`/`routeAttrsNested` entry.
* New names are re-exported unqualified from `Yesod.Core` (and therefore `Yesod`): the classes `ToParentRoute`, `RenderRouteNested` (with its associated types `ParentSite`/`ParentArgs` and its method `renderRouteNested`), and the type/constructor `WithParentArgs`. To avoid clobbering common local bindings, the value-level `toParentRoute` method and the `theParentArgs`/`parentArgsFor` field accessors are *not* re-exported here — import `toParentRoute` from `Yesod.Core.Class.Dispatch.ToParentRoute` if you need it. An application that already defines a top-level `renderRouteNested` (or a type named `ParentArgs`/`ParentSite`) will see an `Ambiguous occurrence` error; resolve it with an explicit import list or qualification.
* The `RouteOpts` constructor (`MkRouteOpts`) is no longer exported from `Yesod.Routes.TH.RenderRoute`; the type stays abstract, as before, and is constructed/modified through `defaultOpts` plus the `set*` functions.
* Several `Yesod.Routes.TH` code-generation entry points changed type beyond the `TyArgs` threading noted below: `mkDispatchClause` takes additional parameters (the accumulated parent dynamic-binder `[Name]` and constructor `[Exp]` lists plus the site's `TyArgs`) and now returns `Q ([String], Clause)` (the child route names that still need nested instances, alongside the clause); `MkDispatchSettings` gained fields for nested-route fallthrough and for selecting the nested dispatch class/function; `mkParseRouteInstance` returns `Q [Dec]` (was `Q Dec`); `mkRouteConsOpts` runs in `Q`; and the `instanceD` helper is no longer exported from `Yesod.Core.Internal.TH`.
* `findOverlapNames` (in `Yesod.Routes.Overlap`) was generalized back from `[ResourceTree String]` to `[ResourceTree t]` — the overlap check never inspected the dynamic-piece type, so it is parametric in it. This widens the accepted argument type; existing call sites are unaffected.
...
* `mkYesodSubDispatchInstance` now requires a parameterized subsite's nested route datatype to declare *exactly* as many type parameters as the subsite has type arguments (previously it accepted a datatype with *more* parameters, which then produced a kind error from generated code). A subsite/nested-datatype arity mismatch is reported as an actionable message at code-generation time. When the nested route datatype's name cannot be resolved (e.g. a typo or missing import), the arity check is skipped and code generation proceeds, so the error surfaces from the generated code rather than from the check.
* Nested-route fallthrough is governed by the module that contains a parent route. A nested dispatch instance (`yesodDispatchNested` / `yesodSubDispatchNested`) always returns `Nothing` when its subtree does not match, leaving the choice between committing to a 404 and falling through to a sibling route to the *delegating* parent clause (per its own `setNestedRouteFallthrough` setting) and ultimately the top-level / subsite terminal authority. Previously a nested instance generated with fallthrough disabled baked a 404 into its own fallback, which — when routes were split across modules — could silently override a fallthrough-wanting parent compiled elsewhere.
* Fix a code-generation failure when a parameterized site that had *not* opted into nested discovery split its nested routes across modules: the delegation probe applied the parent site's type arguments to a child datatype of kind `Type`, aborting the splice with a kind error. The probe now saturates the child by its own reified arity.
* For a nested route fragment whose parent consumes no dynamic pieces (so its `ParentArgs` is `()`), code generation now also emits `UrlToDispatch Child Site` and `RedirectUrl Site Child` instances, letting the fragment's constructor be used directly in `setUrl`/`redirect`. The generated `RedirectUrl Site Child` instance is marked `OVERLAPPABLE` so a hand-written `RedirectUrl` instance for the same child route takes precedence instead of producing a duplicate-instance error. (Routing `redirect SomeR` through the master `RedirectUrl master (Route master)` instance is unaffected, since that is a different head.)
* Edge case (unchanged from previous releases, now documented): a nested parent route with *no* leading path piece compiles to an unconditional first match, so sibling routes declared after it become unreachable under the default `setNestedRouteFallthrough False`. Give the parent at least one static path segment, or enable `setNestedRouteFallthrough True` so a non-matching subtree falls through to the siblings.
* Malformed-route diagnostics raised while parsing a `parseRoutes`/`parseRoutesFile` block (bad overlap markers, stray multipieces, invalid method lists, unbalanced curly brackets, etc.) now go through `MonadFail`'s `fail` rather than a raw `error`, so the failure is attributed to the offending splice instead of surfacing as a bare exception with a call stack.
* New dependency: this release depends on the `encoding` package (introduced with the `HandlerContents` UTF-8 snippet support). `encoding` is not present in Stackage `lts-22.x` or earlier, so stack users on those snapshots will need to add it as an `extra-deps` entry.
* Coordinated release. The previously-released `yesod`, `yesod-form`, `yesod-test`, and `yesod-persistent` cap `yesod-core < 1.7`, so nothing outside the monorepo can build against 1.7 until companion packages with `< 1.8` bounds (already bumped in the monorepo) are published to Hackage. Publish the bumped companions alongside `yesod-core` 1.7.
