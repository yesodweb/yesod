-- | Unit tests for 'generateParseRouteClause' run at @'State' 'Int'@ — the
-- monad-polymorphic nested @parseRouteNested@ clause generator. Production runs
-- it in 'Q' (hygienic 'newName'); here we pin its 'FreshName' supply to a
-- deterministic counter so its 'Clause' output — and the set of parent names it
-- requests a 'ParseRouteNested' instance for — can be asserted directly, with
...
import Test.Hspec
import Control.Monad.State.Strict (evalState, runStateT)
...
-- | Run the nested clause generator at @'State' 'Int'@: deterministic fresh
-- names, pure output. Returns the clause plus the set of parent names it
-- requested a 'ParseRouteNested' instance for.
...
spec :: Spec
spec = describe "generateParseRouteClause (nested parseRoute codegen at State Int)" $ do
    ...
    it "is reproducible: re-running the same tree yields an identical clause (names and all)" $
        -- The State Int supply resets to 0 each run, so the deterministic
        -- binders make the whole Clause '=='. Under Q's 'newName' the uniques
        -- would differ between runs — this equality is exactly what going
        -- through 'FreshName' at 'State' 'Int' buys the tests.
