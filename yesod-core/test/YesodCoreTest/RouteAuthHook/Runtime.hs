...
instance AuthorizeRoute OrgR where
    isAuthorized _ = recordEvent "org" >> pure (Allowed "org")
...
specs = describe "authorization expression hook" $ do
    ...
    it "runs the callback after middleware and before the handler" $
    ...
    it "passes every ancestor capture and the leaf capture to authorization" $
        mapM_ (\path -> check "GET" path 403 Nothing ["before", "middleware", "account", "error"])
